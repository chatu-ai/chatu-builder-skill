...
本 SKILL 里的能力**都按用量计入应用所有者的点数**，且多数都有零成本的简化替代（资料问答 vs 直接塞 prompt、OCR vs 手工填表、工具调用 vs 先查好再喂）。
需求命中这类岔路时，按 `chatu-quickstart`「必须先问用户的三个决策」第 3 条，先给两三个方案 + 推荐项让用户选，再动手——替用户选就是替他花钱，而且数据结构（比如要不要建向量集合）事后换要返工。
